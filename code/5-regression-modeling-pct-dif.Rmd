...
unemploy_test = unemploy_test %>% select(-c(date, black_unemployment, unemployment_rate))
```
...
```{r lm-fit, echo=FALSE}
#run linear regression
lm_fit = lm(percent_dif_unemploy ~ ., data = unemploy_train)
...
ridge_fit = cv.glmnet(percent_dif_unemploy ~ ., alpha = 0, nfolds = 10, data = unemploy_train) 
plot(ridge_fit)
...
lasso_fit = cv.glmnet(percent_dif_unemploy ~ ., alpha = 1, nfolds = 10, data = unemploy_train) 
plot(lasso_fit)
...
mean_dif = unemploy_train %>% summarise(mean(percent_dif_unemploy)) %>% pull()
mean_predictions = rep(mean_dif, nrow(unemploy_test))
mean_RMSE = sqrt(mean((mean_predictions - unemploy_test$percent_dif_unemploy)^2))

# generate linear model predictions and test error
linear_predictions = predict(lm_fit, newdata = unemploy_test)
linear_RMSE = sqrt(mean((linear_predictions - unemploy_test$percent_dif_unemploy)^2))

# generate ridge regression model predictions and test error
ridge_predictions = predict(ridge_fit, 
                            newdata = unemploy_test, 
                            s = "lambda.1se")
ridge_RMSE = sqrt(mean((ridge_predictions - unemploy_test$percent_dif_unemploy)^2))

# generate lasso regression model predictions and test error
lasso_predictions = predict(lasso_fit, 
                            newdata = unemploy_test,
                            s = "lambda.1se")
lasso_RMSE = sqrt(mean((lasso_predictions -  unemploy_test$percent_dif_unemploy)^2))
